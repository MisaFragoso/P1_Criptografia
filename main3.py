...
def get_next_port():
    if 'last_port' not in session:
        session['last_port'] = 5000
    else:
        session['last_port'] += 1
    return session['last_port']
...
def login():
    ...
    # Asigna un nuevo puerto a la sesión
    session['port'] = get_next_port()
    print("Puerto asignado:", session['port'])

    return redirect(f"http://127.0.0.1:{session['port']}/chat")
...
def send_message():
    ...
    recipient = request.form["recipient"]  # Nuevo: Obtener el nombre del destinatario
...
def get_messages():
    ...
    # Convertir los datos relevantes a Base64
    #for message in messages:
     #   message['ciphertext'] = base64.b64encode(message['ciphertext']).decode()
      #  message['tag'] = base64.b64encode(message['tag']).decode()
       # message['nonce'] = base64.b64encode(message['nonce']).decode()
        #message['encrypted_symmetric_key'] = base64.b64encode(message['encrypted_symmetric_key']).decode()
        #message['signature'] = base64.b64encode(message['signature']).decode()
...
if __name__ == "__main__":
    app.run(port=5001, debug=True)  # Modificar el puerto según sea necesario
